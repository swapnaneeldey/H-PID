...
def compute_control_with_diagnostics(x, y, beta, t, energy_function):
    u, x_hat = compute_control(x, y, beta, t, energy_function)
    
    # Compute some statistics
    u_norm = torch.norm(u, dim=1).mean().item()
    x_norm = torch.norm(x, dim=1).mean().item()
    
    return u, u_norm, x_norm, x_hat
...
x_trajectory = torch.zeros(num_steps, batch_size, dim) 

for i, t in enumerate(tt):
    u, u_norm, x_norm, x_hat = compute_control_with_diagnostics(x, y, beta, t, gmm09.energy)
...
for i in range(k):
    x_coords = x_trajectory[:, i*4, 0].numpy()
    y_coords = x_trajectory[:, i*4, 1].numpy()
    
    plt.plot(x_coords, y_coords, '-o', markersize=4, linewidth=4)  # Increased line width
    
    # Add arrows to show direction (every 20 steps for clarity)
    for j in range(0, num_steps-1, 20):
        plt.arrow(x_coords[j], y_coords[j], 
                  x_coords[j+1] - x_coords[j], 
                  y_coords[j+1] - y_coords[j], 
                  shape='full', lw=1, length_includes_head=True,  # Increased arrow line width
                  head_width=0.05, head_length=0.05, color='black', alpha=0.5)
    
    # Mark start and end points
    plt.plot(x_coords[0], y_coords[0], 'go', markersize=18)
    plt.plot(x_coords[-1], y_coords[-1], 'ro', markersize=18)

plt.xlabel('X coordinate', fontsize=24)
plt.ylabel('Y coordinate', fontsize=24)
plt.xlim([-scale*0.6, scale*0.6])
plt.ylim([-scale*0.6, scale*0.6])
plt.grid(True)

# Increase tick label size
plt.tick_params(axis='both', which='major', labelsize=20)

# Ensure axis numbers are visible
plt.gca().tick_params(axis='x', pad=10)  # Add padding to x-axis labels
plt.gca().tick_params(axis='y', pad=10)  # Add padding to y-axis labels

# Custom legend
start_point = mlines.Line2D([], [], color='green', marker='o', markersize=16, linestyle='None', label='Start')
end_point = mlines.Line2D([], [], color='red', marker='o', markersize=16, linestyle='None', label='End')
plt.legend(handles=[start_point, end_point], loc='upper right', fontsize=18)

# Adjust layout to prevent cutting off axis labels
plt.tight_layout()

plt.savefig('x_hat_trajectory.png', dpi=300, bbox_inches='tight')  # Increased DPI for better quality
plt.show()




#x_trajectory = torch.randn(num_steps, batch_size, dim) 
"""
for k in range(batch_size):

    x_coords = x_trajectory[:, k, 0].numpy()
    y_coords = x_trajectory[:, k, 1].numpy()

    # Create the plot
    plt.figure(figsize=(10, 10))
    plt.plot(x_coords, y_coords, '-o', markersize=2)
    plt.title('Evolution of Sample Trajectory')
    plt.xlabel('X coordinate')
    plt.ylabel('Y coordinate')

    # Add arrows to show direction
    for i in range(0, num_steps-1, 10):  # Add an arrow every 10 steps
    ...
    plt.grid(True)
    #plt.show()
    plt.savefig(f'trajectory_{k:04d}.png')
    ...
    if i % 20 == 0:
        ...
        plt.scatter(x_hat[:,0].cpu().numpy(), x_hat[:,1].cpu().numpy(), color='green', alpha=0.9)
