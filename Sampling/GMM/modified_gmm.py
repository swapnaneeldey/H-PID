...
delta_t = tt[1] - tt[0]
beta = torch.tensor(10, device=device)  # Example scalar for beta
batch_size, num_samples, dim = 1000, 500, 2
...
for i, t in enumerate(tqdm(tt)):
    ...
    # Now, the shape of y is [batch_size, num_samples, dim]
    print(y.shape)
    ...
    if i % 10 == 0:
        plt.figure()  # HPID
        # fig, ax = plt.subplots(1, 1, figsize=(8, 8)) #iDEM
        ...
        # plt.plot(x0[:,0].cpu().numpy(), x0[:,1].cpu().numpy(), 'o', color='blue', alpha=0.85) #HPID
        plt.plot(x0[:, 0].cpu().numpy(), x0[:, 1].cpu().numpy(), "o", alpha=0.5)  # HPID
        #    plot_marginal_pair(x0, ax=ax, bounds=plotting_bounds) #iDEM
        #    ax.set_xlim(plotting_bounds)
        #    ax.set_ylim(plotting_bounds)
        #    plt.tight_layout()

        os.makedirs("figs/hpid", exist_ok=True)
        plt.savefig(f"figs/hpid/{i}.png")
        plt.close()

mask_out_of_bounds = (
    (x0[:, 0] < -loc_scaling * 1.4)
    | (x0[:, 0] > loc_scaling * 1.4)  # out of x-bounds
    | (x0[:, 1] < -loc_scaling * 1.4)
    | (x0[:, 1] > loc_scaling * 1.4)  # out of y-bounds
)
num_out_of_bounds = mask_out_of_bounds.sum().item()
print("Number of samples out of bounds:", num_out_of_bounds)
x0_out_of_bounds = x0[mask_out_of_bounds]
print("Samples out of bounds:", x0_out_of_bounds)
...
plt.plot(x1[:, 0].cpu().numpy(), x1[:, 1].cpu().numpy(), "o", color="red", alpha=0.85)
os.makedirs("figs/hpid", exist_ok=True)
plt.savefig(f"figs/hpid/target.png")
plt.close()

print("generated sample : ", x0)
print("target sample : ", x1)

w2 = wasserstein(x0, x1, power=2)
